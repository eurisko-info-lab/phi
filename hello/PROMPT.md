...
│   │   ├── Attributes.scala     # AttrGrammar, FlowDirection
│   │   ├── Validation.scala     # Round-trip checks
│   │   ├── Zipper.scala         # TermZipper navigation
│   │   ├── Change.scala         # Change[A] for edits
│   │   ├── HashConsing.scala    # Content-addressed hashing
│   │   └── Repo.scala           # Pijul-style versioning
│   ├── meta/                    # Generated by: meta2scala(meta.phi)
│   │   ├── Core.scala           # Val, Env, Pat, Expr, Result types
│   │   ├── Lexer.scala          # Tokenization
│   │   ├── Syntax.scala         # Term[A], Syntax[A], Iso
│   │   ├── MetaInterp.scala     # Transform interpreter
│   │   └── gen/                 # Generated operations
│   │       ├── Eval.scala       # Expression evaluation
│   │       ├── Match.scala      # Pattern matching
│   │       ├── Show.scala       # Pretty printing
│   │       └── Interp.scala     # Pattern/substitution
...
│   │   └── TokenRender.scala    # Render context
│   └── user/                    # Generated by: hello2scala(hello.phi)
│       ├── GenBase.scala        # Shared utilities
│       ├── GenHello.scala       # Compiled version
│       ├── GenMeta.scala        # Meta code generation
│       ├── GenPhi.scala         # Phi code generation
...
## Core Infrastructure (~1,340 lines)

| File              | Lines | Purpose                          |
|-------------------|-------|----------------------------------|
| Algebra.scala     | ~397  | Monad, Comonad, RecursionSchemes, Optics, Validated |
| Xform.scala       | ~125  | `Xform[A,B]` trait, registry, Profunctor |
| Attributes.scala  | ~180  | Attribute grammar, flows         |
| Validation.scala  | ~153  | Round-trip checks                |
| Zipper.scala      | ~71   | Term navigation                  |
| Change.scala      | ~105  | Invertible edits                 |
| HashConsing.scala | ~129  | Content-addressed storage        |
| Repo.scala        | ~206  | Pijul-style versioning           |
...
6. Use `Validation.grammarRoundTrip` to verify bidirectionality
7. Use `Xform.roundTrip` to verify transform invertibility
8. Build a structured editor using `TermZipper` and `Change[A]`
9. Track edit history with `Repo[A]` and `Patch[A]`
10. Use recursion schemes (`cata`, `ana`, `hylo`) for generic traversals
11. Compose optics (`Lens`, `Prism`, `Traversal`) for deep access
12. Use `Validated` for parallel error collection in validation passes
